...
/**
 * +----------------------------------------------------------------------
 * | wmt [ 腾讯云存储 开发参考: https://github.com/caojianfei/laravel-qcloud-cos ]
 * +----------------------------------------------------------------------
 ...
 * +----------------------------------------------------------------------
 * | Version: v1.0.0  Date:2019-05-21 Time:15:49
 * +----------------------------------------------------------------------
 */
...
class QcloudCosAdapter extends AbstractAdapter
{
    ...
    public function __construct($prefix = '')
    {
        // TODO: 开发腾讯云文件系统驱动
        $client = new Client([
            'region' => config('filesystems.disks.qcloud-cos.region'),
            'credentials' => config('filesystems.disks.qcloud-cos.credentials'),
            'timeout' => config('filesystems.disks.qcloud-cos.timeout'),
            'connect_timeout' => config('filesystems.disks.qcloud-cos.connect_timeout')
        ]);
        $this->client = $client;
        $this->config = config('filesystems.disks.qcloud-cos');
        $this->setBucket(config('filesystems.disks.qcloud-cos.default_bucket'));
    }
